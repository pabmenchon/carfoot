(function ($) {
    ...
    $('.btn-edit-factor').click(function () {
        let type = ($(this).data('factortable'))
        let factor = ($(this).data('factorid'));

        $.ajax({
            url: 'factors/get_factor',
            method: "post",
            data: {
                type: type,
                factor: factor
            ...
            success: function (res) {
                if (res) {
                    try {
                        ...
                        $('#factor-edit-id').val(factor);
                        $('#type-edit').val(type);
                        if (response[0]) {
                            $('#source-edit').val(response[0]['source']);
                            $('#factor-edit').val(response[0]['factor']);
                            $('#unit-edit').val(response[0]['unit']);
                            $('#reference-edit').val(response[0]['reference']);
                        }
                    ...
                    alert("failed to get factor datas");
                    setTimeout(function () {
        ...
        $('.btn-delete-factor').click(function () {
            let type = ($(this).data('factortable'))
            let factor = ($(this).data('factorid'));
            var cf = confirm("Are you sure to delete this Factor ?");
            if (cf == true) {
                $.ajax({
                    url: 'factors/delete_factor',
                    method: "post",
                    data: {
                        type: type,
                        factor: factor
